...
def _changed_files(root: Path, base: str | None, staged_only: bool = False) -> list[str]:
    files: set[str] = set()
    if staged_only:
        res = _git(root, "diff", "--cached", "--name-only", "--diff-filter=ACMRD")
        return sorted(line for line in res.stdout.splitlines() if line and not line.startswith(".graphtyn/")) if res.returncode == 0 else []
    if base:
    ...
    return sorted(path for path in files if not path.startswith(".graphtyn/"))


def _diff_details(root: Path, base: str | None, changed: list[str], staged_only: bool = False) -> dict[str, dict[str, Any]]:
    details: dict[str, dict[str, Any]] = {}
    commands = []
    if staged_only:
        commands.append(("diff", "--cached", "--no-ext-diff", "--no-color", "--unified=0"))
    elif base:
        commands.append(("diff", "--no-ext-diff", "--no-color", "--unified=0", f"{base}...HEAD"))
    if not staged_only:
        commands.extend([("diff", "--no-ext-diff", "--no-color", "--unified=0"),
...
def analyze_impact(root: Path, graph: dict, base: str | None = None, staged_only: bool = False) -> dict[str, Any]:
    root = root.resolve()
    changed = _changed_files(root, base, staged_only)
    nodes = {n.get("id"): n for n in graph.get("nodes", [])}
    diff_details = _diff_details(root, base, changed, staged_only)
    changed_symbols = []
