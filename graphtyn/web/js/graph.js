...
export function applyFilter() {
      ...
      const showRoute = document.getElementById('f-route')?.checked ?? true;
      const relationInputs = Array.from(document.querySelectorAll('.f-relation'));
      const frameworkLabels = new Set(relationInputs.map(input => input.value));
      const activeRelations = new Set(relationInputs.filter(input => input.checked).map(input => input.value));
      ...
      const filteredNodes = state.fullData.nodes.filter(n => {
        ...
        if (k === 'route' && !showRoute) return false;
        if (state.webFlowNodeIds && !state.webFlowNodeIds.has(n.id)) return false;
        ...
        const searchable = [n.name, n.details, n.path, n.http_method, n.file, n.container, n.namespace, n.id]
      ...
      const filteredLinks = state.fullData.links.filter(l => {
        ...
        return !frameworkLabels.has(l.label) || activeRelations.has(l.label);
      });
...
export function focusWebFlow(nodeId) {
      const labels = new Set(['invoca ruta', 'despacha', 'valida con', 'crea', 'despacha evento']);
      const links = (state.fullData?.links || []).filter(link => labels.has(link.label));
      ...
      while (queue.length) {
        ...
        (adjacent.get(current) || []).forEach(next => {
          if (!visible.has(next)) { visible.add(next); queue.push(next); }
        });
      }
      state.webFlowNodeIds = visible;
      const reset = document.getElementById('reset-web-flow');
      if (reset) reset.style.display = 'block';
      applyFilter();
    }

export function clearWebFlow() {
...
export function onNodeClick(node) {
      ...
      connectedLinks.forEach(l => {
        ...
        const info = (l.label || 'conecta') + ' · ' + String(l.confidence || 'EXTRACTED').toUpperCase();
      ...
      const metadataRows = [
        ...
        node.parser ? ['Resolución', node.parser] : null,
      ].filter(Boolean);
      const metadataBlock = metadataRows.length
        ? '<div class="node-metadata">' + metadataRows.map(row => '<strong>' + escapeHtml(row[0]) + '</strong><span>' + escapeHtml(row[1]) + '</span>').join('') + '</div>'
        : '';
...
export function uninstallReliableNodeDrag() {
      const container = document.getElementById('graph-container');
      const handlers = container && container._graphtynDragHandlers;
      if (!handlers) return;
      ...
      delete container._graphtynDragHandlers;
    }
...
export function installReliableNodeDrag() {
      ...
      container._graphtynDragHandlers = handlers;
