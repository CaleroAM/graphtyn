...
_HIGH_VALUE_OPERATIONS = {
    ...
    "registerdomainevent", "return", "unsign", "sign", "b64decode", "b64encode",
    "timestampsigner", "mark_accessed", "mark_modified", "add_vary_header", "append",
    "badsignature",
}
...
def _select_operations(node: dict, terms: list[str] | None = None, limit: int = 12) -> list[dict]:
    ...
    for index, op in enumerate(node.get("operations") or []):
        ...
        high_value = name.lower() in _HIGH_VALUE_OPERATIONS or op.get("kind") in ("assign", "return", "control", "catch")
        score = term_hits * 100 + int(high_value) * 30 - index * 0.01
        ranked.append((score, index, op))
    ranked.sort(key=lambda item: (-item[0], item[1]))
    mandatory = [item for item in ranked
                 if str(item[2].get("name") or "").lower() in _HIGH_VALUE_OPERATIONS
                 and any(term in f"{item[2].get('name', '')} {item[2].get('text', '')}".lower() for term in terms)]
    chosen = mandatory[:max(2, limit // 2)]
    chosen_keys = {(item[1], item[2].get("kind"), item[2].get("name")) for item in chosen}
    chosen.extend(item for item in ranked
                  if (item[1], item[2].get("kind"), item[2].get("name")) not in chosen_keys)
    selected = [item[2] for item in chosen[:limit]]
...
def _compact_node(node: dict, terms: list[str] | None = None) -> dict:
    ...
    keep = ("id", "name", "kind", "file", "line", "end_line", "signature", "imports",
...
def evidence_result(result: dict, max_nodes: int = 24, max_links: int | None = None) -> dict:
    ...
    selected_type_names = {str(node.get("name") or "") for node in selected if node.get("kind") in ("class", "interface", "struct")}
    emitted_owner_signatures: set[str] = set()
    for node in selected:
        ...
        for key in ("signature", "container", "namespace", "member_type"):
            if node.get(key):
                item[key] = node[key]
        wants_dependencies = any(term.startswith(("dependenc", "import", "using")) for term in evidence_terms)
        if node.get("imports") and wants_dependencies:
            item["imports"] = node["imports"][:16]
        if node.get("semantic_hints"):
            item["hints"] = node["semantic_hints"][:4]
        owner_signature = str(node.get("owner_signature") or "")
        container = str(node.get("container") or "")
        if owner_signature and container not in selected_type_names and owner_signature not in emitted_owner_signatures:
            item["owner_signature"] = owner_signature
            emitted_owner_signatures.add(owner_signature)
        details = str(node.get("details") or "").strip()
        redundant_detail = details == path or details.startswith(("Class en ", "Method en ", "Function en ", "Carpeta: "))
        if details and not redundant_detail:
    ...
    output = {key: value for key, value in result.items()
              if key not in ("matched", "matches", "nodes", "links", "impacted", "estimated_tokens", "intent_terms", "operation_limit")}
    ...
    if matched_ids:
        ...
        if output["complete"]:
            negative = []
            if incoming_calls == 0:
                negative.append("no incoming llama/usa consumers in selected complete graph")
            if implementations == 0:
                negative.append("no implementa relation for matched symbol")
            if inheritance == 0:
                negative.append("no hereda relation for matched symbol")
            if negative:
                output["negative_evidence"] = negative
    requested_checks = []
    if any(term.startswith(("dependenc", "import", "using")) for term in evidence_terms):
        requested_checks.append("dependencies: enumerate imports/usings from evidence")
    if any(term.startswith(("interface", "implement")) for term in evidence_terms):
        requested_checks.append("interfaces: explicitly state whether implementa count is zero")
    if any(term.startswith(("consumer", "caller")) for term in evidence_terms):
        requested_checks.append("consumers: explicitly state whether incoming llama/usa count is zero")
    if requested_checks:
        output["answer_checks"] = requested_checks
        output["guidance"] = str(output.get("guidance") or "") + " Cover every answer_checks item; verbalize zero-valued negative evidence."
    if impacted:
...
def run_mcp_server(workspace: Path, tool_profile: str = "full"):
        ...
        if method == "initialize":
            ...
            if tool_profile == "intent":
                response["result"]["tools"] = [
                    ...
                    if tool["name"] in {"graph_query_intent", "memory_context", "memory_entities", "memory_entity", "memory_topics", "memory_topic", "memory_message_window", "memory_topic_update"}
                ]
            ...
            if name == "graph_query_intent":
                ...
                return _mcp_text(req_id, result)
            elif name in {"memory_entities", "memory_entity", "memory_topics", "memory_topic", "memory_message_window", "memory_topic_update"}:
                ...
                daemon_url = os.environ.get("GRAPHTYN_DAEMON_URL", "http://127.0.0.1:9210").rstrip("/")
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            sys.stderr.write(f"[Graphtyn MCP Error] {e}\n")
