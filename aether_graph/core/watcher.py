"""Portable polling watcher for incremental AetherGraph refreshes."""
...
class ProjectWatcher:
    ...
    def start(self) -> None:
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._run, name=f"aether-watch-{self.root.name}", daemon=True)
        self._thread.start()
...
class WatchManager:
    ...
    def ensure(self, root: Path, index_dir: Path) -> ProjectWatcher:
        ...
        with self._lock:
            ...
            if watcher is None:
                interval = float(os.environ.get("AETHER_WATCH_INTERVAL", "1.0"))
