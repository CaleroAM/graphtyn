...
- Tests: **40 passed** (suites API, MCP, CLI, enriquecimiento, parser e historia):
  - `tests/test_api.py` (10 casos): health, assets servidos, rutas js, config de proyecto, reindex ast_pure + vistas, incremental, diff git, historial, modelos Ollama, lista de proyectos. Guardas de regresión: `favicon.svg`, `export const PALETTES`/`COMM_COLORS` en state.js e `import { state }` en dashboard.js.
